...
echo "[1/5] 停止容器..."
docker rm -f openclaw-gateway openclaw-fallback mihomo-tun 2>/dev/null || true
...
crontab -l 2>/dev/null | grep -v -E 'openclaw|watchdog|health-check|trends|audit|sync|cert-check|kbase' | crontab - 2>/dev/null || true
...
echo "[5/5] 清理临时文件..."
rm -f /data/etc/openclaw/docker-compose.yml /tmp/openclaw-compose.yml 2>/dev/null || true
